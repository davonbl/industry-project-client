            ...
            <div className="basket__card">
                <div className="basket__container">
                    ...
                    <div className="basket__label-container">
                        ...
                        <p className="basket__adjusted">{calculatePercentage(baseJson[0].housing, housing)}</p>
                    </div>
                    <div className="basket__label-container">
                        <h3>Grocery/Food: ${clickedZip.food} </h3>
                        <p className="basket__adjusted">{calculatePercentage(baseJson[0].food, food)}</p>
                    </div>
                    <div className="basket__label-container">
                        <h3>Transportation: ${clickedZip.transportation}</h3>
                        <p className="basket__adjusted">{calculatePercentage(baseJson[0].transportation, transportation)}</p>
                    </div>
                    <div className="basket__label-container">
                        <h3>Health Care: ${clickedZip.healthcare}</h3>
                        <p className="basket__adjusted">{calculatePercentage(baseJson[0].healthcare, healthcare)}</p>
                    </div>
                    <div className="basket__label-container">
                        <h3>Child Care: ${clickedZip.childcare}</h3>
                        <p className="basket__adjusted">{calculatePercentage(baseJson[0].childcare, childcare)}</p>
                    </div>
                </div>
                <div className="basket__label-container">
                    <h2 className="basket__total">Total: ${zipTotal} </h2>
                    <p className="basket__adjusted">{calculatePercentage(totalBase, zipTotal)}</p>
                </div>
